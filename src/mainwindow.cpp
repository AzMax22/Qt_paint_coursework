...
{
    DialogInfoProgram w_info_prog(nullptr);
    w_info_prog.setModal(true);
    w_info_prog.exec();
}


void MainWindow::on_info_img_triggered()
{
    if (image.wasInitialized() == false){
        QMessageBox::warning(nullptr,"Oшибка","Вы не выбрали изображение" );
        return;
    ...
    //DialogInfoImg w_info_img(nullptr, image.getWidth(), image.getHeight(),
      //                       image.getBitDepth(), image.getTypeColor(),
        //                     image.getPathToImg());
    DialogInfoImg w_info_img(nullptr, image);
    w_info_img.setModal(true);
    w_info_img.exec();
}


void MainWindow::on_merge_img_triggered()
{
    if (image.wasInitialized() == false){
        QMessageBox::warning(nullptr,"Oшибка","Вы не выбрали изображение" );
        return;
    ...
    QString fileName = QFileDialog::getOpenFileName(this, "Cлить файл",
                                                    "/home",
                                                     "PNG_Images (*.png )");

    Image_PNG second_img;

    second_img.readFromDisk(fileName);

    image.mergeImg(second_img);
...
void MainWindow::on_change_bright_triggered()
{
    if (image.wasInitialized() == false){
        QMessageBox::warning(nullptr,"Oшибка","Вы не выбрали изображение" );
        return;
    ...
    BrightWindow w_bright(nullptr, image);
    w_bright.setModal(true);
    w_bright.exec();
...
void MainWindow::on_merge_horiz_triggered()
{
    if (image.wasInitialized() == false){
        QMessageBox::warning(nullptr,"Oшибка","Вы не выбрали изображение" );
        return;
    ...
    image.mergeImgNearby(second_img);
