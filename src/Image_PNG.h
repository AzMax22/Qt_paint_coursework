...
private:
    ...
    QColor getColorPixel(int x,int y); //если координаты вне изб. возвращает белый цвет //только для RGBA
    int findMax(int a,int b);
public:
    //Конструктор
    Image_PNG();

    Image_PNG(int width, int height, png_byte color_type, png_byte bit_depth, QString path_to_img);//конструктор с выделением памяти под изб.
    ...
    QString getTypeColor();//получить тип цвета изб.

    int getBitDepth();//получить глубину цвета изб.

    QString getPathToImg();//получить путь к изб.
    ...
    void mergeImg(Image_PNG second_img);

    void changeBright(float coef);

    void mergeImgNearby(Image_PNG second_img);
